"""Selecting one reported fact out of many that all look right.
...
from lazytools.financials.period import ResolvedWindow
...
class Fact:
    ...
    accession: str
    form: str
    #: When the filing that carries this fact was filed. ``None`` when the
    #: source does not publish one — an ESEF repository dates a report by the
    #: period it covers and by when the repository ingested it, neither of which
    #: is a filing date. Left absent rather than substituted, because every
    #: version policy is a statement about filing order and a made-up date would
    #: make that ordering look decided.
    filed: date | None
    fy: int | None = None
    fp: str | None = None
    frame: str | None = None
...
def pick(
    ...
    *,
    ...
    undated = [f for f in candidates if f.filed is None]
    if undated:
        # Every policy here is an ordering by filing date. Without one there is
        # no order, and inventing a tiebreak (accession? source order?) would
        # dress an arbitrary pick as a version decision. One value is still an
        # answer; several are a conflict nothing here can resolve.
        values = {f.value for f in candidates}
        if len(values) > 1:
            raise AmbiguousFactError(
                f"{len(candidates)} candidates for {candidates[0].concept} carry no filing "
                f"date and report different values ({sorted(values)}); no version policy "
                "can order them",
