...
ELEMENTS: dict[str, ElementSpec] = {
    ...
    "capex_intangibles": ElementSpec("duration", "Cash paid for intangibles and capitalised development. Excluded from a capex figure, it inflates free cash flow.", sign="magnitude"),
    "house_capex": ElementSpec("duration", "The capital spend counted here. No maintenance/growth split is claimed: filings do not disclose one.", sign="magnitude"),
    ...
    "finance_lease_total": ElementSpec("instant", "Finance lease liabilities. Often already inside reported debt, where adding them again double counts.", sign="magnitude"),
    "operating_lease_total": ElementSpec("instant", "Operating lease liabilities, current and non-current. One component alone is a floor.", sign="magnitude"),
    "house_adjusted_debt": ElementSpec("instant", "Debt as this house counts it. The lease convention must be stated: two major agencies capitalise leases and one does not, so the figures are not comparable across conventions.", sign="magnitude"),
    ...
    "readily_available_cash": ElementSpec("instant", "Cash less disclosed restrictions. Says nothing about which legal entity holds it or whether it can reach the debt.", sign="magnitude"),
