...
OPTIMIZER_SPECIALIST_SYSTEM = (
    ...
    "OR whenever the task hands you Black-Litterman views -- portfolio_optimizer_* "
    "has NO view support at all; portfolio_tree_* does, via a node's "
    "constraints.views/view_tau (a single node with no children is a valid flat "
    "portfolio there too, so this is the only path for 'flat pool, but with "
    "views'). Views shift only the node's expected return (view_covariance_policy "
    "defaults to 'prior_risk'), so they do nothing for objective='min_risk'/'hrp' "
    "-- use max_ratio/max_return/max_utility to actually see a view move the "
    "solved weights, and say so plainly if asked for min_risk/hrp with views. "
