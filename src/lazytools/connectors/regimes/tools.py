    ...
    def as_tools(self) -> list[Any]:
        ...
        descriptions = {
            ...
            "regime_load_from_datahub": (
                ...
                "symbols — for MULTIPLE symbols pass ONE comma-separated "
                "string 'SPY,TLT,GLD' (over MCP a JSON list is NOT accepted "
                "and a single token like 'SPY,TLT' would be read as one "
                "symbol only if the comma were omitted — always use the comma "
                "to separate); a single symbol is just 'SPY'. All symbols must "
                "have overlapping history at the chosen frequency or the load "
                "fails with 'no rows with complete data' — widen start/end, "
                "use a coarser frequency, or pass fillna='ffill'. start, end "
                "(ISO dates, optional); frequency (default 'W'); field (price "
                "field, default 'adj_close'); fillna (missing-value policy, "
                "default 'none'); data_key (depot key, default 'datahub'). "
                "Returns: data_key, n_rows, n_cols, columns, date_range, "
                "source."
            ),
        }
