...
!!! info "Status & install"
    **Status: alpha.** Install the Gmail extra:
    ```bash
    pip install "lazytoolkit[gmail] @ git+https://github.com/selvaz/LazyTools.git"   # adds google-api-python-client, google-auth, google-auth-oauthlib
    ```
...
| Symptom | Cause | Fix |
|---|---|---|
| `ImportError: requires the 'gmail' extra` | Google libs not installed | `pip install "lazytoolkit[gmail] @ git+https://github.com/selvaz/LazyTools.git"` |
