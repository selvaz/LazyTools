...
The `EdgarTools` ToolProvider was removed (audit CA-03): agents reach SEC
data through the hub-backed `datahub_*` tools. The client remains as
injectable plumbing for non-agent code.

::: lazytools.connectors.edgar.EdgarClient

## Market data (transport client only)

`MarketDataTools` was removed for the same reason; use `datahub_*`.
