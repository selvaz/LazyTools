...
!!! info "Status & install"
    **Status: alpha.** Install the Telegram extra:
    ```bash
    pip install "lazytoolkit[telegram] @ git+https://github.com/selvaz/LazyTools.git"   # adds httpx
    ```
...
| Symptom | Cause | Fix |
|---|---|---|
| `ImportError: requires the 'telegram' extra` | `httpx` not installed | `pip install "lazytoolkit[telegram] @ git+https://github.com/selvaz/LazyTools.git"` |
